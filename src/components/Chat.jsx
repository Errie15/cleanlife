...
const Chat = ({ messages, users, currentUserId, onSendMessage }) => {
  ...
  return (
    <div className="flex flex-col h-full relative mb-1">
      ...
      {messages.length === 0 ? (
        // Visa bara ett inputfält när det inte finns några meddelanden
        <form onSubmit={handleSubmit} className="flex bg-white p-3 rounded-xl shadow-md chat-input-container border border-purple-100 mt-2">
          <button
            type="button"
        ...
        <>
          ...
          <form onSubmit={handleSubmit} className="flex bg-white p-3 rounded-b-xl shadow-md chat-input-container border-t-0 border border-purple-100">
            <button
              type="button"
              onClick={() => setShowEmoji(!showEmoji)}
              className="p-2 rounded-full bg-purple-100 text-purple-700 mr-3 hover:bg-purple-200 transition-colors"
